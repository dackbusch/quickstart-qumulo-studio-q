...
This Quick Start builds a Qumulo cluster and one or more editorial workstations all hosted on AWS with options for 1 TB and 12 TB usable capacity. Workstations are built on the AWS G4dn family of Amazon Elastic Cloud Compute (Amazon EC2) instances with NVIDIA T4 graphics processing units (GPUs) and a Teradici PC over IP (PCoIP) agent on the workstation. You can optionally add more workstations as a post-deployment step. 
