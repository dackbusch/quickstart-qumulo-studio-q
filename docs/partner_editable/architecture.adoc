...
*** The AD server also provides all DNS services local to the VPC forwarding public requests to R53.
** A provisioner instance that configures:
*** Qumulo cluster users, roles, IP failover, an SMB share, and joins the cluster to the AD domain.
*** Tags all EBS volumes with the Stack Name and modifies the KMS Customer Managed Key (CMK) policy if a CMK is specified in the template.
