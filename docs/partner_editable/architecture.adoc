...
*** 12-TB usable capacity option contains 20 of 100-GiB Amazon EBS gp2 volumes and 40 of 500-GiB Amazon EBS Throughput Optimized HDD (st1) volumes.
...
*** Tags all Amazon EBS volumes with the CloudFormation stack name and modifies the AWS Key Management Service (AWS KMS) key policy if a KMS key is specified in the template.
** An Active Directory domain controller and local Domain Name System (DNS) server hosted on an EC2 M5 (m5.large) instance. All workstations and the Qumulo cluster are joined to the Active Directory domain automatically. The Active Directory server also provides all DNS services local to the VPC while forwarding public requests to Amazon Route 53.
